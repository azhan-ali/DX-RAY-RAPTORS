...
function generateRecommendations(dims: DimensionResult[], ci: Record<string, unknown>, test: Record<string, unknown>, doc: Record<string, unknown>): Recommendation[] {
  ...
  for (const d of sorted) {
    ...
    const isCritical = d.score < 40;
    const isWarn = d.score < 70;
    if (d.id === "ci_build") recs.push({ id: id++, title: ci.hasCI ? (isCritical ? "Fix CI pipeline failures" : "Optimize CI build performance") : "Add CI/CD pipeline", description: ci.hasCI ? (isCritical ? "CI pipeline has critical reliability issues. Fix failing steps and add build caching." : "CI builds can be optimized with caching, parallel jobs, and conditional workflows.") : "No CI detected. Adding GitHub Actions automates testing and catches bugs early.", impact: isCritical ? "critical" : isWarn ? "high" : "medium", effort: ci.hasCI ? "medium" : "medium", devHoursSaved: isCritical ? 8.5 : isWarn ? 5.2 : 3.0, dimension: "ci_build" });
    if (d.id === "test_stability") recs.push({ id: id++, title: test.hasTests ? (isCritical ? "Fix critical test gaps" : "Improve test coverage") : "Add automated tests", description: test.hasTests ? `${test.testFileCount} test files found. ${isCritical ? "Coverage is critically low — add tests for core modules." : "Increase coverage and add integration tests."}` : "No tests detected. Adding tests prevents regressions and builds confidence.", impact: isCritical ? "critical" : "high", effort: "high", devHoursSaved: test.hasTests ? (isCritical ? 12.0 : 6.4) : 12.0, dimension: "test_stability" });
    if (d.id === "commit_velocity") recs.push({ id: id++, title: isCritical ? "Revive development velocity" : "Improve commit frequency", description: isCritical ? "Near-zero commit activity detected. Unblock contributors with smaller PRs, clearer issues, and faster reviews." : "Commit frequency is below healthy levels. Consider smaller PRs, feature flags, and trunk-based development.", impact: isCritical ? "critical" : "high", effort: "low", devHoursSaved: isCritical ? 8.0 : 4.0, dimension: "commit_velocity" });
    if (d.id === "code_review") recs.push({ id: id++, title: isCritical ? "Establish code review workflow" : "Streamline review process", description: isCritical ? "Limited PR workflow detected. Enforce branch protection, require reviews, and add CODEOWNERS." : "Review turnaround can be improved. Add CODEOWNERS, set SLAs, and use auto-assign for reviewers.", impact: isCritical ? "critical" : "high", effort: "low", devHoursSaved: isCritical ? 7.3 : 4.5, dimension: "code_review" });
    if (d.id === "doc_freshness") recs.push({ id: id++, title: doc.hasReadme ? (isCritical ? "Overhaul project documentation" : "Refresh documentation") : "Add project README", description: doc.hasReadme ? (isCritical ? "Documentation is severely outdated. Schedule a doc sprint and add contributing guidelines." : "Documentation needs refreshing. Update README, add CHANGELOG, and document setup steps.") : "No README found. A good README reduces onboarding time by up to 60%.", impact: isCritical ? "critical" : "medium", effort: "low", devHoursSaved: isCritical ? 5.0 : 2.8, dimension: "doc_freshness" });
  }
...
function fallbackPatches(dims: DimensionResult[], tree: GHTreeItem[], ci: Record<string, unknown>, test: Record<string, unknown>): GhostPatch[] {
  ...
  const sorted = [...dims].sort((a, b) => a.score - b.score).filter(d => d.score < 80);
  for (const d of sorted.slice(0, 5)) {
    ...
    else if (d.id === "test_stability") patches.push({ id: id++, title: "Add test coverage reporting", file: ".github/workflows/ci.yml", language: "yaml", diff: "+      - name: Run tests with coverage\n+        run: npm test -- --coverage\n+      - name: Upload coverage report\n+        uses: codecov/codecov-action@v4\n+        with:\n+          fail_ci_if_error: false", impact: "Track test coverage trends", confidence: 82, patchType: "test_setup" });
    if (d.id === "commit_velocity") patches.push({ id: id++, title: "Add PR template for faster merges", file: ".github/pull_request_template.md", language: "markdown", diff: "+## What changed?\n+<!-- Brief description -->\n+\n+## Why?\n+<!-- Link to issue or explain motivation -->\n+\n+## Testing\n+- [ ] Unit tests pass\n+- [ ] Manual QA done\n+\n+## Checklist\n+- [ ] Code follows project style\n+- [ ] Docs updated if needed", impact: "Reduce PR cycle time ~35%", confidence: 80, patchType: "velocity_boost" });
    ...
    else if (d.id === "doc_freshness") patches.push({ id: id++, title: "Add CONTRIBUTING guide", file: "CONTRIBUTING.md", language: "markdown", diff: "+# Contributing\n+\n+## Getting Started\n+1. Fork the repository\n+2. Create a feature branch\n+3. Make your changes\n+4. Run tests: `npm test`\n+5. Submit a PR\n+\n+## Code Style\n+- Follow existing patterns\n+- Add tests for new features\n+- Update docs when needed", impact: "Improve contributor onboarding", confidence: 85, patchType: "documentation" });
  }
